...
#                 shutil.rmtree(file_path, onerror=readonly_handler)
#             else:
#                 os.remove(file_path)
        ...
        if (match
            ...
            if typ == 'apk':
                ...
                if db_entry.exists() and not rescan:
                    ...
                    destination = Path(settings.BASE_DIR) / ".." / "scripts" / "manifests"
                    print("Destination:", destination)
                    if not os.path.exists(destination):
                        os.makedirs(destination)
                    try:
                        shutil.copyfile(mani_file, destination / (app_dic['app_name'][:-3] + "xml"))
                    ...
                    ###
                    # elf analysis
                    # 主要包括了apk中的二进制文件有几个，有没有开启一些二进制防护，可以省略
                    # elf_dict = elf_analysis(app_dic['app_dir'])
                    elf_dict = {'elf_analysis': [], 'elf_strings': []}
                    # cert analysis
                    # disabled manually
                    # cert_dic = cert_info(
                    #     app_dic['app_dir'],
                    ...
                    # tracker analysis
                    # 创建Trackers对象的时候会自动更新本地的数据库文件，配置好要检测的trackers
                    ...
                    # quark analysis, disabled in settings.py
                    ...
                    # 在这个函数里面会分别去提取配置文件中的secret，url，email，以及elf里面的一些内容
                    ...
                    # file_path = r"\\?\{}".format(app_dic['app_dir'])
                    # print(file_path)
                    # print(app_dic['app_dir'])
                    os.system("rm -rf " + Path(settings.UPLD_DIR).as_posix())
                    os.system("mkdir -p " + Path(settings.UPLD_DIR).as_posix())
                    # del_file_or_folder(file_path)
